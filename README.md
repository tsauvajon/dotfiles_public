...
the missing SSH key, fills `git.signingKey` with its public key path, prints
upload commands, then builds and activates the Home Manager generation.
...
- `~/.ssh/id_ed25519.pub`: SSH authentication and Git signing public key,
  generated by `scripts/bootstrap-keys.sh` when missing.
