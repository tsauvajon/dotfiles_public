...
fn run_check(
    ...
    // Compare skills directory symlinks
    let real_skills = paths.private_build.join("opencode/skills");
    let shadow_skills = shadow_build.join("opencode/skills");
    if real_skills.is_dir() && shadow_skills.is_dir() {
        let collect_skill_links = |dir: &std::path::Path| -> BTreeMap<String, String> {
        ...
        let real_links = collect_skill_links(&real_skills);
        let shadow_links = collect_skill_links(&shadow_skills);
