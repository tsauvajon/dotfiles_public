...
- `opencode-imports/*/package.json`: staged package manifests are regenerated by `setup.sh` from their source repos and then merged into the OpenCode package manifest.
...
6. OpenCode package manifests: update `config/opencode/package.json` or private/imported `package.json` files, then run `bash setup.sh` so activation runs Bun install.
