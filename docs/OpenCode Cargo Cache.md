...
- `CC=opencode-sccache-clang`
- `CXX=opencode-sccache-clang++`

The `opencode-sccache-clang*` wrappers are Home Manager-installed macOS helpers
that resolve the active Xcode or Command Line Tools compiler through `xcrun` and
call it through `sccache`. They avoid multi-word `CC` / `CXX` values, which some
native build tools cannot execute directly.

`CC` and `CXX` are only injected when both wrappers are already on `PATH`. They
apply to all native compiler invocations in that Cargo workspace, not only Cargo
build scripts. To disable just this native layer, launch OpenCode with
`OPENCODE_CARGO_NATIVE_CACHE=0`.
