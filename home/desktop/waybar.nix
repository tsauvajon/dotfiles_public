...
#
# `inputs.catppuccin-waybar` is the source-only upstream repo. We mount
# its `themes/` directory at `catppuccin/themes/` so `mocha.css`,
# `frappe.css`, etc. land at the path the SCSS import expects.
{
  pkgs,
  lib,
  inputs,
...
lib.mkIf pkgs.stdenv.isLinux {
  xdg.configFile = {
    ...
    "waybar/catppuccin/themes".source = "${inputs.catppuccin-waybar}/themes";
