...
# 5. Bootstraps the per-machine SSH key from the private git identity and
#    fills git.signingKey with its public key path when safe.
# 6. Installs public Homebrew packages on macOS before any Nix builds.
# 7. Installs missing Arch packages from packages/arch/pacman.txt when
#    pacman is available.
# 8. Builds homeConfigurations.<host>.activationPackage from this
...
if [ ! -f "$private_ref/flake.nix" ]; then
  ...
  # after generating the SSH key). Skip the "edit and rerun" exit and
...
On the next run, setup.sh will generate a missing SSH key, fill
...
# Install the public Brewfile before bootstrap or Home Manager can build Rust
...
if [ "$(uname -s)" = "Darwin" ]; then
  ...
  if command -v brew >/dev/null 2>&1; then
    ...
    if [ -f "$public_brewfile" ]; then
      printf '==> Installing public Homebrew packages from %s\n' "$public_brewfile"
      brew bundle install --file="$public_brewfile"
    fi
  else
    printf '\n'
    printf 'warning: Homebrew not found; skipping managed packages. To install manually, run:\n'
...
if [ "$(uname -s)" = "Darwin" ] && [ "$brew_available" -eq 1 ]; then
  ...
  if [ -f "$personal_brewfile" ]; then
    printf '==> Installing personal Homebrew packages from %s\n' "$personal_brewfile"
    brew bundle install --file="$personal_brewfile"
  fi
